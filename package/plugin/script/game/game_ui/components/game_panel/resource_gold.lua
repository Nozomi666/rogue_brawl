--------------------------------------------------------------------------------------
class.resource_gold = extends(class.panel) {

    new = function(parent)
        -- 底图是一张黑色的图片 这个可以写死
        local panel = class.panel.new(parent, nil, 1, 1, 1, 1)
        -- 改变panel对象的类
        panel.__index = class.resource_gold
        panel:construct()
        panel:draw()
        panel:show()
        return panel
...
function mt:updateBoard()
    ...
    local amt = math.floor(p:getStats(RES_GOLD))
    if amt >= 100000 then
