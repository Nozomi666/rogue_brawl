...
local function on_tick()
	...
	for i = cur_index + 1, #q do
		-- gdebug(str.format('on tick: %d', i))
		local callback = q[i]
		cur_index = i
		q[i] = nil
		if callback then
			-- gdebug(str.format('on callback: %d', i))
			m_wakeup(callback)
		end
	end
	cur_index = 0
...
function ac.timerReportErr()
	print('error - 计时器防夹断错误提示', debug.traceback())
end

function ac.instantTimer(timeout, count, on_timer)

    on_timer(nil, count)

    if count > 1 then
        return ac.timer(timeout, count - 1, on_timer)
    end
