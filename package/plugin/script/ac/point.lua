...
-- 检测是否可行走
function mt:isWalkable()
    jass.MoveLocation(point.dummy, self[1], self[2])
    local detectBlock = false
    glo.udg_hasDestructable = false
    EnumDestructablesInCircleBJ(100, point.dummy, code.testWalkableDestructable)

    return not glo.udg_hasDestructable
    -- return not IsTerrainPathable(self[1], self[2], PATHING_TYPE_WALKABILITY)
end
...
function mt:effect(data)
    local eff = ac.point_effect(self, data)
    if data.time then
        ac.wait(data.time * 1000, function()
            if eff then
                eff:remove()
            end
        end)
...
-- 创建一个点
--------------------------------------------------------------------------------------
function mt:new(x, y, z)
    -- print("ac.point called")
    return setmetatable({x, y, z}, point)
end

--------------------------------------------------------------------------------------
function mt:getUnitLoc(u)
    if not u or not u.handle then
        return mt:new(0, 0, 0)
    end

    return mt:new(GetUnitX(u.handle), GetUnitY(u.handle), 0)

end

--------------------------------------------------------------------------------------
function mt:getJUnitLoc(u)

    return mt:new(GetUnitX(u), GetUnitY(u), 0)

end

--------------------------------------------------------------------------------------
function mt:polarPoint(source, distance, angle)
    local x, y = source:get()

    return mt:new(x + distance * math.cos(angle), y + distance * math.sin(angle))
end
--------------------------------------------------------------------------------------
fc.polarPoint = function(source, distance, angle)
    local x, y = source:get()
    return mt:new(x + distance * math.cos(angle), y + distance * math.sin(angle))
end
--------------------------------------------------------------------------------------
fc.convertJPoint = function(pointJ)
    return mt:new(GetLocationX(pointJ), GetLocationY(pointJ), 0)
end
--------------------------------------------------------------------------------------
function mt:convertJPoint(pointJ)
    return mt:new(GetLocationX(pointJ), GetLocationY(pointJ), 0)
end
--------------------------------------------------------------------------------------
function mt:inRect(r)
    local x, y = self:get()
